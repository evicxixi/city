...
# CourseCategory,CourseSubCategory,DegreeCourse,Teacher,Scholarship,Course,CourseDetail,OftenAskedQuestion,CourseOutline,CourseChapter,CourseSection,Homework,PricePolicy
...
class DegreeCourse(models.Model):
    ...
    total_scholarship = models.PositiveIntegerField(
        verbose_name="总奖学金(贝里)", default=40000)  # 2000 2000
...
class Teacher(models.Model):
    ...
    # 用于GenericForeignKey反向查询，不会生成表字段，切勿删除
    # teacher_price_policy = GenericRelation("PricePolicy")
...
class Course(models.Model):
    ...
    sub_category = models.ForeignKey(
        "CourseSubCategory", on_delete=models.CASCADE)
    ...
    degree_course = models.ForeignKey(
        "DegreeCourse", blank=True, null=True, help_text="若是学位课程，此处关联学位表", on_delete=models.CASCADE)
...
class OftenAskedQuestion(models.Model):
    ...
    content_type = models.ForeignKey(
        ContentType, on_delete=models.CASCADE)  # 关联course or degree_course
...
class CourseChapter(models.Model):
    ...
    course = models.ForeignKey(
        "Course", related_name='coursechapters', on_delete=models.CASCADE)
...
class CourseSection(models.Model):
    ...
    chapter = models.ForeignKey(
        "CourseChapter", related_name='coursesections', on_delete=models.CASCADE)
    name = models.CharField(max_length=128)
...
class Coupon(models.Model):
    ...
    coupon_type = models.SmallIntegerField(
        choices=coupon_type_choices, default=0, verbose_name="券类型")

    money_equivalent_value = models.IntegerField(verbose_name="等值货币")
    off_percent = models.PositiveSmallIntegerField(
        "折扣百分比", help_text="只针对折扣券，例7.9折，写79", blank=True, null=True)
    minimum_consume = models.PositiveIntegerField(
        "最低消费", default=0, help_text="仅在满减券时填写此字段")

    content_type = models.ForeignKey(
        ContentType, blank=True, null=True, on_delete=models.CASCADE)
    object_id = models.PositiveIntegerField(
        "绑定课程", blank=True, null=True, help_text="可以把优惠券跟课程绑定")
    content_object = GenericForeignKey('content_type', 'object_id')

    quantity = models.PositiveIntegerField("数量(张)", default=1)
    open_date = models.DateField("优惠券领取开始时间")
    close_date = models.DateField("优惠券领取结束时间")
    valid_begin_date = models.DateField(
        verbose_name="有效期开始时间", blank=True, null=True)
    valid_end_date = models.DateField(
        verbose_name="有效结束时间", blank=True, null=True)
    # coupon_valid_days = models.PositiveIntegerField(verbose_name="优惠券有效期（天）", blank=True, null=True,
    #                                                 help_text="自券被领时开始算起")
    date = models.DateTimeField(auto_now_add=True)

    class Meta:
        verbose_name_plural = "31. 优惠券"
...
class CouponRecord(models.Model):
    ...
    account = models.ForeignKey(
        "Account", verbose_name="拥有者", on_delete=models.CASCADE)
    ...
    # verbose_name="关联订单")  # 一个订单可以有多个优惠券
    ...
    def __str__(self):
        return '%s-%s-%s-%s' % (self.account, self.number, self.status, self.coupon)
...
class EnrolledCourse(models.Model):
    ...
    course = models.ForeignKey("Course", limit_choices_to=~Q(
        course_type=2), on_delete=models.CASCADE)
    ...
    order_detail = models.OneToOneField(
        "OrderDetail", on_delete=models.CASCADE)  # 使订单购买后支持 课程评价
...
class ScoreRule(models.Model):
    ...
    rule = models.SmallIntegerField(
        choices=score_rule_choices, verbose_name="积分规则")
    score_type_choices = ((0, '奖励'), (1, '惩罚'), (2, '初始分配'))
    score_type = models.SmallIntegerField(
        choices=score_type_choices, verbose_name="奖惩", default=0)
...
class ScoreRecord(models.Model):
    ...
    degree_course = models.ForeignKey(
        "DegreeCourse", blank=True, null=True, verbose_name="关联学位课程", on_delete=models.CASCADE)

    score_rule = models.ForeignKey(
        "ScoreRule", verbose_name="关联规则", on_delete=models.CASCADE)
    account = models.ForeignKey(
        "Account", verbose_name="被执行人", on_delete=models.CASCADE)
    ...
    received_score = models.IntegerField(
        "实际到账金额贝里)", help_text="仅奖励用", default=0)
    # balance = models.PositiveIntegerField(verbose_name="奖金余额(贝里)")

    maturity_date = models.DateField("成熟日期(可提现日期)")

    applied = models.BooleanField(
        default=False, help_text="奖赏纪录是否已被执行", verbose_name="是否已被执行")
    applied_date = models.DateTimeField(
        blank=True, null=True, verbose_name="事件生效日期")
...
class StudyRecord(models.Model):
    ...
    enrolled_degree_course = models.ForeignKey(
        "EnrolledDegreeCourse", on_delete=models.CASCADE)
    course_module = models.ForeignKey(
        "Course", verbose_name="学位模块", limit_choices_to={'course_type': 2}, on_delete=models.CASCADE)
...
class DegreeRegistrationForm(models.Model):
    """学位课程报名表"""
    enrolled_degree = models.OneToOneField(
        "EnrolledDegreeCourse", on_delete=models.CASCADE)
    ...
    stu_specified_mentor = models.CharField(
        "学员自行指定的导师名", max_length=32, blank=True, null=True)
    study_plan_choices = ((0, "1-2小时/天"),
                          (1, "2-3小时/天"),
                          (2, "3-5小时/天"),
                          (3, "5小时+/天"),
                          )
    study_plan = models.SmallIntegerField(
        choices=study_plan_choices, default=1)
...
class EnrolledDegreeCourse(models.Model):
    ...
    enrolled_date = models.DateTimeField(auto_now_add=True)
    valid_begin_date = models.DateField(
        verbose_name="有效期开始自", blank=True, null=True)  # 开通第一个模块时，再添加课程有效期，2年
    valid_end_date = models.DateField(
        verbose_name="有效期结束至", blank=True, null=True)
    status_choices = (
        (0, '在学中'),
        (1, '休学中'),
        (2, '已毕业'),
    ...
    mentor_fee_balance = models.PositiveIntegerField(
        "导师费用余额", help_text="这个学员的导师费用，每有惩罚，需在此字段同时扣除")
    order_detail = models.OneToOneField(
        "OrderDetail", on_delete=models.CASCADE)  # 使订单购买后支持填写报名表
...
class Order(models.Model):
    ...
    payment_number = models.CharField(
        max_length=128, verbose_name="支付第3方订单号", null=True, blank=True)

    order_number = models.CharField(
        max_length=128, verbose_name="订单号", unique=True)  # 考虑到订单合并支付的问题
    account = models.ForeignKey("Account", on_delete=models.CASCADE)
    actual_amount = models.FloatField(verbose_name="实付金额")

    status_choices = ((0, '交易成功'), (1, '待支付'), (2, '退费申请中'),
                      (3, '已退费'), (4, '主动取消'), (5, '超时取消'))
    status = models.SmallIntegerField(
        choices=status_choices, verbose_name="状态")
    date = models.DateTimeField(auto_now_add=True, verbose_name="订单生成时间")
    pay_time = models.DateTimeField(blank=True, null=True, verbose_name="付款时间")
    cancel_time = models.DateTimeField(
        blank=True, null=True, verbose_name="订单取消时间")
...
class OrderDetail(models.Model):
    ...
    content_type = models.ForeignKey(
        ContentType, on_delete=models.CASCADE)  # 可关联普通课程或学位
    object_id = models.PositiveIntegerField()
    content_object = GenericForeignKey('content_type', 'object_id')
...
class TransactionRecord(models.Model):
    ...
    content_type = models.ForeignKey(
        ContentType, blank=True, null=True, on_delete=models.CASCADE)
    object_id = models.PositiveIntegerField(
        blank=True, null=True, verbose_name="关联对象")
    content_object = GenericForeignKey('content_type', 'object_id')

    transaction_number = models.CharField(
        unique=True, verbose_name="流水号", max_length=128)
    date = models.DateTimeField(auto_now_add=True)
...
class HomeworkRecord(models.Model):
    ...
    student = models.ForeignKey(
        "EnrolledDegreeCourse", verbose_name="学生", on_delete=models.CASCADE)
    score_choices = ((100, 'A+'),
                     (90, 'A'),
                     (85, 'B+'),
                     (80, 'B'),
    ...
    score = models.SmallIntegerField(
        verbose_name="分数", choices=score_choices, null=True, blank=True)
    ...
    mentor_comment = models.TextField(
        verbose_name="导师批注", blank=True, null=True)  # 导师
    status_choice = (
        (0, '待批改'),
        (1, '已通过'),
        (2, '不合格'),
    ...
    status = models.SmallIntegerField(
        verbose_name='作业状态', choices=status_choice, default=0)
    ...
    reward_status = models.SmallIntegerField(
        verbose_name='作业记录奖惩状态', default=0)
...
class StuFollowUpRecord(models.Model):
    ...
    enrolled_degree_course = models.ForeignKey(
        "EnrolledDegreeCourse", verbose_name="学生", on_delete=models.CASCADE)
    mentor = models.ForeignKey("Account", related_name='mentor', limit_choices_to={
                               'role': 1}, verbose_name="导师", on_delete=models.CASCADE)
    followup_tool_choices = ((0, 'QQ'), (1, '微信'), (2, '电话'), (3, '系统通知'))
    followup_tool = models.SmallIntegerField(
        choices=followup_tool_choices, default=1)
    record = models.TextField(verbose_name="跟进记录")
    attachment_path = models.CharField(
        max_length=128, blank=True, null=True, verbose_name="附件路径", help_text="跟进记录的截图等")
    date = models.DateTimeField(auto_now_add=True)
